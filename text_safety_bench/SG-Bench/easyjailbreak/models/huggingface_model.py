...
class Qwen3VLModel(WhiteBoxModelBase):
    """
    Wrapper for Qwen3-VL models (Qwen3VLForConditionalGeneration).

    Uses AutoProcessor for tokenization and chat template application.
    The ``tokenizer`` attribute holds the processor, which provides
    ``apply_chat_template``, ``batch_decode``, and other tokenizer-compatible
    methods.  Text-only inputs are used for text safety evaluation; multimodal
    inputs are supported by passing ``images`` / ``videos`` to ``generate``.
    """
    ...
    def clear_conversation(self):
        pass  # stateless — no conversation history is kept

    def create_conversation_prompt(self, messages, clear_old_history=True):
        """Build a list[dict] chat representation from alternating user/assistant strings."""
        if isinstance(messages, str):
            messages = [messages]
        chat = []
        for index, message in enumerate(messages):
            role = "user" if index % 2 == 0 else "assistant"
            chat.append({"role": role, "content": [{"type": "text", "text": message}]})
        return chat
    ...
    def generate(self, messages, clear_old_history=True, **kwargs):
        ...
        :param messages: A single user string, a list of alternating
            user/assistant strings, or a pre-built list[dict] chat.
        :param kwargs: Forwarded to ``model.generate()``.  Keys recognised
            by the processor (``images``, ``videos``) are extracted before
            calling the model.
        ...
        if isinstance(messages, list) and messages and isinstance(messages[0], str):
            system_msg = getattr(self, '_system_message', None)
            chat = []
            if system_msg:
                chat.append({"role": "system", "content": [{"type": "text", "text": system_msg}]})
            for index, msg in enumerate(messages):
                role = "user" if index % 2 == 0 else "assistant"
                chat.append({"role": role, "content": [{"type": "text", "text": msg}]})
        else:
            chat = messages  # already a list[dict]

        # Extract multimodal kwargs the processor expects
    ...
    def batch_generate(self, conversations, **kwargs) -> List[str]:
        return [self.generate(conv, **kwargs) for conv in conversations]

    # ------------------------------------------------------------------
    # WhiteBoxModelBase interface
    # ------------------------------------------------------------------
    ...
    def format(self, **kwargs):
        # Minimal implementation: return prompt value directly.
...
def from_pretrained_qwen3vl(
    ...
    **generation_config: Dict[str, Any],
    ...
    """
    Load a Qwen3-VL model and its processor, returning a :class:`Qwen3VLModel`.

    :param str model_name_or_path: HuggingFace repo id or local path.
    :param str model_name: Friendly name stored on the wrapper (default ``"qwen3-vl"``).
    :param Optional[torch.dtype] dtype: Model dtype; defaults to ``torch.bfloat16``.
    :param generation_config: Extra keyword arguments forwarded to ``model.generate()``.

    **Example**::

        target_model = from_pretrained_qwen3vl(
    ...
    return Qwen3VLModel(model, processor, model_name=model_name, generation_config=generation_config)